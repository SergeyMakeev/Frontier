#pragma once
// Compile-time configuration, diagnostics policy, and the host integration
// context (allocation + task parallelism).
//
// Everything here is designed to be overridden by the embedding project by
// defining the macro BEFORE including any hlod header:
//
//   #define HLOD_FATAL(msg) MyEngine::Panic(msg)   // exceptions-off builds
//   #include "hlod/world.h"

#include <cstddef>
#include <cstdint>
#include <cstdlib>

#define HLOD_VERSION_MAJOR 0
#define HLOD_VERSION_MINOR 3
#define HLOD_VERSION_PATCH 0
#define HLOD_VERSION_STRING "0.3.0"

// ---------------------------------------------------------------------------
// Diagnostics
//
// HLOD_FATAL fires on a TRUE CONTRACT VIOLATION only — a caller bug that no
// correct program can trigger (attaching to a node that is not an expansion
// point, submitting non-finite bounds, deforming a shared asset). It must not
// return.
//
// Expected runtime races are NOT fatal and never route here: a handle that
// went stale because its page was collected mid-stream makes mutating calls
// quiet no-ops and queries report absent. That distinction is the whole
// reason streaming callers can be written without locks.
//
// The default throws, which keeps the contract tests expressive. Projects
// building without exceptions should define HLOD_FATAL to abort or to their
// own panic handler.
// ---------------------------------------------------------------------------

#ifndef HLOD_FATAL
  #include <stdexcept>
  #include <string>
  #define HLOD_FATAL(msg) throw ::std::logic_error(::std::string("hlod: ") + (msg))
#endif

#define HLOD_CHECK(cond, msg)                                                  \
    do                                                                         \
    {                                                                          \
        if (!(cond)) HLOD_FATAL(msg);                                          \
    } while (false)

// Invariants that are expensive to check and cannot fail for a correct
// library: compiled out in release builds.
#ifndef HLOD_ASSERT
  #ifdef NDEBUG
    #define HLOD_ASSERT(cond, msg) ((void)0)
  #else
    #define HLOD_ASSERT(cond, msg) HLOD_CHECK(cond, msg)
  #endif
#endif

namespace hlod {

// ---------------------------------------------------------------------------
// Host context: allocation and optional task parallelism.
//
// Page blobs, asset storage and per-worker output buffers are allocated
// through this. The World's small per-call scratch stays in std::vector: it
// amortizes to zero allocations after the first few frames, so routing it
// through the context would buy nothing and cost a lot of template churn.
// ---------------------------------------------------------------------------

using AllocFn = void* (*)(size_t bytes, size_t alignment, void* user);
using FreeFn  = void (*)(void* ptr, void* user);

// Invoked as fn(i, payload) for i in [0, count). An implementation may run
// the calls on any thread and in any order, but must not return until all of
// them have completed. The default runs them serially on the calling thread.
using ParallelForFn = void (*)(uint32_t count, void (*fn)(uint32_t i, void* payload),
                               void* payload, void* user);

void* defaultAlloc(size_t bytes, size_t alignment, void* user);
void  defaultFree(void* ptr, void* user);
void  defaultParallelFor(uint32_t count, void (*fn)(uint32_t i, void* payload),
                         void* payload, void* user);

struct HlodContext
{
    AllocFn       alloc       = &defaultAlloc;
    FreeFn        free        = &defaultFree;
    ParallelForFn parallelFor = &defaultParallelFor;

    // Upper bound on how many of parallelFor's calls can run concurrently.
    // Used to size per-worker output buffers; 1 keeps selectCut serial no
    // matter what parallelFor does.
    uint32_t workerCount = 1;

    void* user = nullptr;
};

// Process-wide default (malloc/free, serial). Returned by reference so it can
// be handed to anything expecting a context.
const HlodContext& defaultContext();

} // namespace hlod
