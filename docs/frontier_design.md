...
## 2. Subtrees have an implicit parent

A `Subtree` is a reusable immutable descendant forest. Its serialized index
zero is an implementation anchor used for packed child ranges; it is not a
renderable node and is never returned. At runtime its direct renderable nodes
are children of the node on which the definition is mounted.

This is the only legal way to instantiate a subtree. A mount therefore always
has one valid renderable parent: either a TLAS root or an extendable node in
another placement.

## 3. Definitions form a DAG; placements form trees

An extendable `NodeDesc` stores a `SubtreeKey` and local translation/uniform
scale. Definition references may form a directed acyclic content graph: many
city nodes can reference the same house key. Runtime mounting turns each
reference into a separate placement, so each top-level instance still owns an
ordinary tree.

The database requires keys to match and rejects a direct self-reference at
build time. Content pipelines are responsible for rejecting longer dependency
cycles before attempting to stream them.

## 4. Immutable data and instance data

One serialized `Subtree` permanently owns:
...
- dependency keys and expansion transforms.
...
6. a transformed mounted definition fits inside the expansion node's authored
   bound.
...
available. A missing mounted definition stops both cuts at its extendable
parent while exposing the target key to the streamer.

Plain leaves emit directly. Interior and extendable nodes are decided by
...
Incorrect live operations remain contract failures: key mismatch, invalid
transform, non-extendable parent, duplicate child, or bound escape.
...
| register/release unused definition | O(1), excluding blob destruction |
